...
def add_figure_box(doc, image_filename, fig_num, fig_title, fig_recommendation):
    ...
    r_rec_label = rec_p.add_run("Caption Description: ")
...
def create_document():
    ...
    add_figure_box(
        ...
        fig_num=1,
        fig_title="Complete System Architecture and ROS 2 Communication Pipeline for KUKA Manipulator over Ethernet KRL Interface.",
        fig_recommendation="High-resolution architecture diagram showing the five-layer pipeline from multimodal human inputs through MoveIt 2 Pilz planning and Ethernet KRL communication to the KUKA KR6 manipulator and vacuum end-effector."
    )
    ...
    add_figure_box(
        ...
        fig_num=2,
        fig_title="Twelve-Marker ArUco Constellation Layout and Perspective-Corrected Workspace Homography Calibration.",
        fig_recommendation="Calibration debug visualization showing detected ArUco marker bounding boxes with identifier tags, reference coordinate axes, and segmented colored target cubes on the physical table."
    )
    ...
    add_figure_box(
        ...
        fig_recommendation="RViz MoveIt 2 simulation scene illustrating the three-dimensional KUKA KR6 manipulator model, planned linear and point-to-point trajectory paths, collision table environment, and end-effector frame."
    )
    ...
    output_path = os.path.join(WORKSPACE_ROOT, "Journal", "KUKA_ROS2_Conference_Paper_VLA_Comparison.docx")
    try:
        doc.save(output_path)
        print(f"Successfully generated VLA Comparison Word document at: {output_path}")
    except PermissionError:
        fallback_path = os.path.join(WORKSPACE_ROOT, "Journal", "KUKA_ROS2_Conference_Paper_VLA_Comparison_New.docx")
