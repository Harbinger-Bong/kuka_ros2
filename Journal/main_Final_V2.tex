...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.92\textwidth]{../images/figure4_latency_chart.png}
...
To establish a rigorous paradigm contrast, Table~\ref{tab:vla_comparison} and the radar chart in Fig.~\ref{fig:radar_chart} evaluate our proposed deterministic modular ROS 2 framework against published empirical metrics of leading Vision-Language-Action models including OpenVLA~\cite{openvla2024}, Octo~\cite{octo2024}, and RT-2~\cite{rt2_2023}.
...
\toprule
\textbf{Evaluation Metric} & \textbf{Proposed Modular ROS 2} & \textbf{OpenVLA~\cite{openvla2024} / Octo~\cite{octo2024} Models} & \textbf{Literature Evidence \& Basis} \\
\midrule
...
\addlinespace
\textbf{Hardware \& Compute} & CPU-only execution ($<$\SI{500}{\mega\byte} RAM, 0 MB GPU VRAM) & High-end GPU clusters required ($\ge$\SI{16}{\giga\byte} VRAM, $>$\SI{300}{\watt}) & OpenVLA requires A100 GPU (Kim et al., 2024); our pipeline deploys directly onto standard factory IPCs. \\
\addlinespace
\textbf{Cartesian Accuracy} & High accuracy ($3.18 \pm \SI{0.33}{\milli\meter}$ mean error) & Coarse accuracy (\SI{10.0}{\milli\meter} to \SI{25.0}{\milli\meter} typical error) & Published OpenVLA tabletop error is 12--20 mm; sub-3.5 mm is required for airtight vacuum seal. \\
\addlinespace
\textbf{Trajectory Safety} & Deterministic collision-free planning (Pilz LIN / PTP)~\cite{pilz_motion2021} & Stochastic policy rollout with potential trajectory drift & Pilz planner enforces linear paths; VLA policies exhibit unconstrained trajectory jitter. \\
\addlinespace
\textbf{Controller Support} & Native Ethernet KRL XML bridge (KUKA KRC4 Port 54600)~\cite{kuka_eki2019} & Requires low-level direct joint velocity interfaces & Direct compatibility with commercial industrial controllers without warranty voiding. \\
\addlinespace
\textbf{Semantic Flexibility} & Structured vocabulary with phonetic alias dictionaries & Open-vocabulary unconstrained natural language instructions & VLA models excel at zero-shot novel object reasoning (Brohan et al., 2023). \\
\bottomrule
\end{tabular}
\end{table*}% -------------------------------------------------------------------------
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.92\textwidth]{../images/figure6_vla_benchmark_comparison.png}
